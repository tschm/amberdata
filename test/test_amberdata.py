...
def test_history():
    ...
    with pytest.raises(HTTPError):
        AmberRequest(key="a").price_history(pair="eth_usd")
...
def test_history_mock_2():
    with requests_mock.Mocker() as m:
        ...
        f = AmberRequest(key="a").price_history(pair="eth_usd")
...
def test_history_ohlcv(get_ohlcv):
    for exchange, data in AmberRequest(key="a").ohlcv_history(pair="eth_usd", startDate=pd.Timestamp("2020-01-15"), endDate=pd.Timestamp("2020-01-17"), timeInterval=TimeInterval.HOURS, exchange="bitfinex"):
        pass
...
def test_history_bid_ask(get_bid_ask):
    for exchange, data in AmberRequest(key="a").bid_ask_history(pair="eth_usd", startDate=pd.Timestamp("2020-01-12"), endDate=pd.Timestamp("2020-01-13"), exchange="bitfinex"):
        pass
...
def test_history_bid_ask_2():
    with requests_mock.Mocker() as m:
        ...
        d = {exchange: data for exchange, data in AmberRequest(key="a").bid_ask_history(pair="eth_usd", startDate=pd.Timestamp("2020-01-12"), endDate=pd.Timestamp("2020-01-13"), exchange="bitfinex")}
...
def test_history_mock(get_history):
    ...
    AmberRequest(key="a").price_history(pair="eth_usd", startDate=pd.Timestamp("2020-01-12"), endDate=pd.Timestamp("2020-01-13"), timeInterval=TimeInterval.HOURS)
